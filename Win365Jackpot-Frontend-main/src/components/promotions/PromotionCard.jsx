...
/**
 * PromotionCard
 * Purely presentational — takes one `promotion` object (see
 * src/data/promotions.js) and renders it. Compatible as-is with a live
 * `/api/promotions` response as long as field names are preserved.
 */
export default function PromotionCard({ promotion, onClaim, onViewDetails }) {
  return (
    <motion.div
    ...
    >
      ...
      <div className="relative h-36 overflow-hidden">
        <img
          src={promotion.casinoImage}
          alt={promotion.casinoName}
        ...
        <div className="absolute bottom-3 left-4 flex items-center gap-2">
          <img
            src={promotion.casinoLogo}
            alt=""
            className="w-8 h-8 rounded-full object-cover border"
            style={{ borderColor: 'rgba(212,175,55,0.5)' }}
          />
          <span className="text-white/80 text-xs font-body font-semibold">{promotion.casinoName}</span>
        </div>
      </div>
      ...
      <div className="p-5 flex flex-col flex-1 gap-3">
        ...
        <div className="flex items-start gap-1.5 text-xs font-body text-white/50">
          ...
          <span>{promotion.bonusDetails}</span>
        </div>
