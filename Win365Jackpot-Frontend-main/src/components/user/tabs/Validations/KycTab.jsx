// Player KYC — same workflow, same endpoints, same admin approval process as
// the Affiliate portal's KycTab.jsx (backend KYCSubmission model is shared;
// kyc_type is auto-derived server-side from whether the account has an
// affiliate_profile). Kept as a parallel component rather than a shared one
// only because it reads from the user session (authFetch) instead of the
// affiliate session (affiliateFetch).
...
function Card({ children, style = {} }) {
  return (
    <div style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${C.border}`, borderRadius: 16, padding: 20, ...style }}>
      {children}
    </div>
  );
}
...
export default function KycTab({ onToast }) {
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    const res = await authFetch(`${API}/api/kyc/submit/`, { method: "POST", body: fd });
    setSubmitting(false);
    if (res?.ok) {
  ...
  if (kycStatus === "approved") {
    return (
      <StatusBanner
        ...
        title="KYC Verified"
        message={`Your KYC was approved${status.reviewed_at ? ` on ${fmtD(status.reviewed_at)}` : ""}. You're all set.`}
      />
    );
  }
      ...
      {neverSubmitted && kycStatus !== "rejected" && (
        <Card style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.6)" }}>
            KYC not submitted yet. Verifying your identity unlocks full deposits and withdrawals.
          </div>
        </Card>
      )}
