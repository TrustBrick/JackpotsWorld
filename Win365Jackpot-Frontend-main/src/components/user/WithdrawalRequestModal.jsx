...
export default function WithdrawalRequestModal({ onClose, onSuccess, onError }) {
  ...
  // Casino Wallet Available Balance for whichever casino is currently
  // selected — refetched every time casinoId changes.
  ...
  useEffect(() => {
    if (!casinoId) { setAvailable(null); return; }
    let cancelled = false;
    setLoadingBalance(true);
    authFetch(`${API}/api/wallet/withdrawal-requests/casino-balance/?casino_id=${casinoId}`)
      .then(r => r.json())
      .then(j => { if (!cancelled) setAvailable(Number(j.available) || 0); })
  ...
  const amountError =
    amount === "" || !casinoId || loadingBalance ? "" :
    Number.isNaN(amountNum) ? "Enter a valid amount." :
    amountNum <= 0 ? "Amount must be greater than zero." :
    amountNum > available ? `Amount exceeds this casino's available balance of ${fmt(available)}.` :
    "";

  const canContinue = amount !== "" && !amountError && casinoId && methodCode && !loadingBalance;

  const handleSubmit = async () => {
    setSubmitting(true);
    setError("");
    try {
      const res = await authFetch(`${API}/api/wallet/withdrawal-requests/create/`, {
        method: "POST",
        body: JSON.stringify({ amount, notes, casino_id: casinoId, method_code: methodCode }),
      });
    ...
    >
      ...
      >
        ...
        <div style={{ textAlign: "center" }}>
          ...
          <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)" }}>From your selected casino wallet</div>
        </div>
        ...
        {loadingOptions ? (
          ...
          <>
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", borderRadius: 10, background: "rgba(96,165,250,0.1)", border: `1px solid ${C.blue}30` }}>
              <AlertCircle size={14} style={{ color: C.blue, flexShrink: 0 }} />
              <span style={{ fontSize: 11.5, color: "rgba(255,255,255,0.65)", lineHeight: 1.5 }}>
                Your request will be reviewed by our team before the amount is paid out.
              </span>
            </div>
          ...
          <>
            <div>
              ...
              >
                <option value="" style={{ background: "#14161f" }}>Select a country</option>
                {countries.map(c => (
            ...
            {casinoId && (
              <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", textAlign: "center" }}>
                Casino wallet available balance: <b style={{ color: C.green }}>{loadingBalance ? "…" : fmt(available)}</b>
              </div>
            )}
