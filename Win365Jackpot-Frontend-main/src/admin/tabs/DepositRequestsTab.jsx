...
import { RefreshCw, Search, Eye, X, Check, Clock, Ban, Download, AlertCircle } from "lucide-react";
...
export default function DepositRequestsTab({ onToast }) {
  ...
  const loadSummary = useCallback(() => {
    adminFetch(`${API}/api/admin-panel/deposit-requests/summary/`)
      .then(r => r?.json())
  ...
  const loadList = useCallback(() => {
    ...
    adminFetch(`${API}/api/admin-panel/deposit-requests/?${params}`)
      .then(r => r?.json())
  ...
  const handleExport = () => {
    ...
    adminFetch(`${API}/api/admin-panel/deposit-requests/export/?${params}`)
      .then(r => r?.blob())
      .then(blob => {
        if (!blob) return;
        const url = URL.createObjectURL(blob);
        const a = document.createElement("a");
        a.href = url; a.download = "deposit_requests.csv"; a.click();
  ...
  const SUMMARY_CARDS = [
    ...
    { key: "approved", label: "Approved", icon: Check, color: C.green },
    { key: "rejected", label: "Rejected", icon: X, color: C.red },
    { key: "cancelled", label: "Cancelled", icon: Ban, color: C.muted },
    { key: "approved_today", label: "Approved Today", icon: Check, color: "#A78BFA" },
  ];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      ...
      <Table
        headers={["Reference", "User", "User ID", "Amount", "Casino", "Payment Method", "Status", "Requested", ""]}
        loading={loading}
        colSpan={9}
        emptyText="No deposit requests in this view"
      >
        {items.map(row => (
          <tr key={row.id} {...rowHover(C)} style={{ borderBottom: `1px solid ${C.border}` }}>
            ...
            <td style={{ padding: "11px 14px", fontSize: 11.5, fontFamily: "monospace" }}>{row.user_uid}</td>
            <td style={{ padding: "11px 14px", fontSize: 12.5, fontFamily: "monospace", fontWeight: 700, color: C.gold }}>{fmt(row.amount)}</td>
            <td style={{ padding: "11px 14px", fontSize: 12.5 }}>{row.casino_name || "—"}</td>
...
function DepositDetailModal({ id, C, onClose, onDone }) {
  ...
  const load = useCallback(() => {
    setLoading(true);
    adminFetch(`${API}/api/admin-panel/deposit-requests/${id}/`)
      .then(r => r?.json())
  ...
  const act = async (action, body = {}) => {
    setBusy(true);
    const r = await adminFetch(`${API}/api/admin-panel/deposit-requests/${id}/${action}/`, {
  ...
  const canApprove = status === "pending" || status === "processing";
  const canProcessing = status === "pending";
      ...
      <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 540, maxHeight: "86vh", display: "flex", flexDirection: "column" }}>
        <Card style={{ padding: 0, overflow: "hidden", display: "flex", flexDirection: "column" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 18px", borderBottom: `1px solid ${C.border}` }}>
            <div style={{ fontSize: 14, fontWeight: 800, color: C.text }}>Deposit Request Review</div>
            ...
            {loading || !data ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  ...
                  <DetailField C={C} label="Payment Method" value={data.method_label || "—"} />
                  <DetailField C={C} label="Casino" value={data.casino_name || "—"} />
                  <DetailField C={C} label="Payment Reference" value={data.payment_reference || "—"} />
                ...
                {!mode && (
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap", borderTop: `1px solid ${C.border}`, paddingTop: 14 }}>
                    ...
                    {!canApprove && !canProcessing && !canReject && (
