 ...
 * Poker tournaments — nested inside PokerManageTab as a view toggle rather
 * than a separate top-level admin tab.
 */
export default function PokerRegistrationsTable({ onToast }) {
  ...
  const load = useCallback(() => {
    setLoading(true);
    adminFetch(`${API}/api/admin-panel/poker/registrations/`)
      .then(r => r?.json())
  ...
  const patch = async (id, body) => {
    const r = await adminFetch(`${API}/api/admin-panel/poker/registrations/${id}/`, {
      method: "PATCH",
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ fontSize: 13, color: C.muted }}>
          {items.length} registration{items.length !== 1 ? "s" : ""} total
        </div>
        <Btn outline small onClick={load}><RefreshCw size={12} /> Refresh</Btn>
      </div>

      <Table
        headers={["User", "UID", "Tournament", "Casino", "Event Date", "Phone", "Email", "Registered", "Status", "Note"]}
        loading={loading}
        colSpan={10}
        emptyText="No poker interest registrations yet"
      >
        {items.map(item => (
          <tr key={item.id} {...rowHover(C)} style={{ borderBottom: `1px solid ${C.border}` }}>
            ...
            <td style={{ padding: "11px 14px", fontSize: 12.5 }}>{item.tournament_name}</td>
